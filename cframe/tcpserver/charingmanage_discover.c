...
static void * deal_charing_manage_discover_event( void * args ){
    ...
    dzlog_info( "DISCOVER 守护进程开启通讯通道, 开始监听DISCOVER 消息" );
    int sock;
...
void check_charing_handler( void ){
	...
	while( ppNode ){
	    ...
	    if( ppNode->count == 6 ){
			...
			dzlog_warn( "心跳已6次，客户端IP(%s),用户(%s) 已经掉线!", ppNode->peerip, ppNode->name );
            safe_message_node( ppNode->name );
            dzlog_debug( "完成连接链表卸载(%s)!", ppNode->name );            
            safe_heart_node( ppNode->name );
            dzlog_debug( "完成心跳包连接卸载(%s)!", ppNode->name );
		}else if( ppNode->count > 0 ){		
            //dzlog_warn("客户端IP(%s),用户(%s) 出现异常!", ppNode->peerip,ppNode->name );
		    ppNode->count ++;
		    //dzlog_debug("count(%d)", ppNode->count);//查看计数器内容
	    }else if( ppNode->count == 0 ){
		    ppNode->count++;
		    //dzlog_debug("count(%d)", ppNode->count);//查看计数器内容
	    }
...
HEART_INFO_T * create_heart_information_list_node_ext( int fd, char * hostname, char * ipaddr ){
    ...
    if( strlen( hostname ) + 1 < sizeof( node->hostname ) ){
        ...
        info( "hostname(%s)  保存值过大(%d)", node->hostname, strlen( hostname ) );
    }

    if( strlen( ipaddr ) + 1 < sizeof( node->peerip ) ){
        memcpy( node->peerip, ipaddr , strlen( ipaddr ) + 1 );
    }else{
        dzlog_warn( "ipaddr(%s)  保存值过大   (%d)", ipaddr, strlen( ipaddr ) );
    }
